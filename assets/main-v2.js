...
//   $('span[data-hex-code] input').on('change keyup blur', function(e) {
//     if (e.key != 'Backspace') {
//       pickr.setColor( '#' + $(this).val() )
//       pickrMob.setColor( '#' + $(this).val() )
//     }
//   })

  pickr.on('change', (color, source, instance) => {
    $('.hype-cropper__body--image, span[data-hex-color]').css('background', color.toHEXA().toString())
    $('span[data-hex-code] input').val(color.toHEXA().toString().replace('#', ''))
  })
...
//     document.querySelectorAll('[data-method]').onclick = function(event) {
    $('body').on('click', '[data-method]', function (event) {
        ...
        var input;
        var data;
        var Color = $('.hype-cropper__body--image').css('background-color') || '#e5e5e5';
            ...
            switch (data.method) {
                ...
                case 'getCroppedCanvas':
                    if (result) {
                        ...
                        $('.main_product_slider [data-slick-index=0]').find('div.ex1')
...
//                                 $(".btns-wrap .new-btn span").html("Replace File");
                              ...
                              $('.main_product_slider [data-slick-index=0]').find('div.ex1').css('opacity', '0.3')
                              setTimeout(function() {
                                $(".main_product_slider .slider-for__item img[data-index='1']")
                                  .parent().fadeIn();
                                
                                $('.main_product_slider [data-slick-index=0]').find('div.ex1').css('opacity', '1')
                                
                                $('.main_product_slider [data-slick-index=0]').find('div.ex1').find('img').attr({class: 'has-new-zoom', 'data-magnify-src': ''})
                                
                                let g = $('.main_product_slider [data-slick-index=0]').find('div.ex1').find('img').position()
                                ...
                                $('.main_product_slider [data-slick-index=0]').find('div.ex1').
                                get(0).style.setProperty('--pos-bottom', posBot + 'px')

                                $('.main_product_slider [data-slick-index=0]').find('div.ex1').
                                get(0).style.setProperty('--pos-top', posTop + 'px')

                                $('.main_product_slider [data-slick-index=0]').find('div.ex1').addClass('custom')
...
//                                 }, 500);
                                ...
                                $(".main_product_slider .slider-for__item img[data-index='1']").css({
                                    "width": "94%"
                                });
                                $(".main_product_slider .slider-for__item img[data-index='1']").parent().css({
                                    "background-color": "#ebebeb"
                                });

                                var height = $(".main_product_slider .slider-for__item img[data-index='2']").parent().height();
                                console.log($(".main_product_slider .slider-for__item img[data-index='1']").parent().css({
                                    "min-height": height + "px"
                                }));

                                //console.log($(".slider-for__item img[data-index='1']").css({"box-shadow":"0px 4px 4px 0px rgba(0,0,0,0.27)"}));
                                console.log($(".main_product_slider .slider-for__item img[data-index='1']").css({
                                    "box-shadow": "0px 5px 12px -4px rgba(0,0,0,0.46)"
                                }));
                                if ($(window).width() <= 768) {
                ...
                reader.onload = function(e) {
                    ...
                    image.onload = function() {
                       ...
                       if ( width <= 499 || height <= 499 ) {
                            const text = 'File resolution too low. Please upload an image with minimum 500px resolution.'                          
                            displayCropperErrorPopup( 'Upload Failed', text, 'warning warning--red' )
...
//     })
  ...
  $('body').on( 'click touchstart touchmove mousemove', '.cropper-canvas img, .img-magnifier-glass', function(e) {
    const workIMG = document.querySelector('.cropper-canvas img')
...
function displayCropperErrorPopup( heading, text, type = 'error', trigger = null ) {
  $('.hype-cropper-popup__body')
  .find('h3').text( heading )
  .parent().find('p').text( text )
  .parent().attr('class', 'hype-cropper-popup__body ' + type)

  
  if ( type == 'error' ) {
    $('.hype-cropper-popup__foot')
    .html('<button class="btn btn--tertiary btn--black" data-close-cropper-popup>Cancel</button><button class="btn" data-reset-cropper>Delete</button>')
  } else {
    ...
    $('.hype-cropper-popup__foot')
    .html('<button class="btn btn btn--tertiary" '+ attr +' data-reset-cropper>Close</button>')
  }
