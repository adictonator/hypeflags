...
theme.Search = (function () {
	...
	function validateSearchResultForm(evt) {
		var isInputValueEmpty =
			this.$searchResultInput.val().trim().length === 0
...
theme.Cart = (function () {
	...
	Cart.prototype = _.assignIn({}, Cart.prototype, {
		...
		_onEditClick: function (evt) {
			...
			if ($updateLine.hasClass(config.showClass)) {
				$updateLine
					.nextAll('tr.cart__update-wrapper')
					.first()
...
theme.Product = (function () {
	...
	Product.prototype = _.assignIn({}, Product.prototype, {
		...
		_initAddToCart: function () {
			...
			$(self.selectors.productForm, self.$container).on(
				'submit',
				function (evt) {
					evt.preventDefault()

					var $addToCart = $(
						self.selectors.addToCart,
						self.$container
					)
					...
					if (isInvalidQuantity) {
						self._showErrorMessage(
							self.errors.quantityMinumumMessage
						)
					} else {
						var $data = $(
							self.selectors.productForm,
							self.$container
						)
						var redirectUrl = $data.data(
							'product-form-redirect-url'
						)
		...
		_updateAddToCart: function (evt) {
			...
			if (variant) {
				if (variant.available) {
					$(this.selectors.addToCart, this.$container).prop(
						'disabled',
						false
					)
		...
		$.get(recommendationsSectionUrl).then(
			function (section) {
				...
				if (recommendationsMarkup.trim() !== '') {
					...
					this.$container.find('.skeleton-loader-cust').remove()
				}
			}.bind(this)
		)
	}
			...
			sendTrekkieEvent: function () {
				...
				window.ShopifyAnalytics.lib.track(
					'Product Recommendations Displayed',
					{
						...
						numberOfRecommendationsDisplayed:
							numberOfRecommendationsDisplayed,
					}
				)
			},
...
$(document).ready(function () {
	...
	$('.slider-nav').on('init', function () {
		$('.slider-wrapper').find('.slick-arrows').show()
	})

	$('.slider-for').on('init', function (event, slick) {
		$('.slider-for, .slider-nav').css('display', 'block')

		//     $('.main_product_slider img').load( function(){
		//         $(this).removeClass('_mhh')
		//     	$(this).find('.skeleton-loader-cust').remove()
		//     })
	})

	//   $('.slider-nav').on('init', function(event, slick){
	//      $('.slider-nav-wrapper img').load( function(){
	//         $(this).removeClass('_oof')
	//     	$(this).find('.skeleton-loader-cust').remove()
	//     })
	//   })
...
$(document).ready(function () {
	...
	//noToShow = Math.round($( window ).width() / 280)

	//     if($('.slider_jss').length)
	//     {
	//       $('.slider_jss').slick({
	...
	//         //slidesToScroll: noToShow,
	//         centerMode: true,
	//         cssEase: 'linear',
	//         arrows: true,
	//         draggable: true,
	//         prevArrow: '<button class="slide-arrow arrow-prev"><svg aria-hidden="false" width="16px" height="16px" viewBox="0 0 30 30" transform="rotate(0)" class="Icon-sc-1ny8wc9-0 dneFCZ"><path d="M13.245 9.862L2.97 20.138l1.989 1.988 10.276-10.275 9.806 9.807 1.989-1.989L15.235 7.874z"></path></svg></button>',
	//         nextArrow: '<button class="slide-arrow arrow-next"><svg aria-hidden="false" width="16px" height="16px" viewBox="0 0 30 30" transform="rotate(0)" class="Icon-sc-1ny8wc9-0 dneFCZ"><path d="M13.245 9.862L2.97 20.138l1.989 1.988 10.276-10.275 9.806 9.807 1.989-1.989L15.235 7.874z"></path></svg></button>',
	...
	const f = $('.product-card__image-with-placeholder-wrapper').height()
	$('button.slide-arrow').css('top', f / 2 - 16 + 'px')
})
	...
	const swiper = new Swiper('.swiper-best-seller', {
		...
		on: {
			...
			init: function () {
				...
				$('img.grid-view-item__image').load(function () {
					const _p = $(this)
