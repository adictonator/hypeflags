...
function ColorPicker(element, color) {
  console.log('asdasd2222', element)
    ...
    element.addEventListener('click', function(event) {
        ...
        if (event.target.getAttribute('data-color') && event.target.getAttribute('data-color') != 'undefined') {
...
(function() {
    const HTMLContent = `
		<svg id="color_box" width="185" height="185">
			<defs>
			...
			<rect rx="5" ry="5" x="1" y="1" width="100%" height="185" stroke="#fff" stroke-width="2" fill="url(#pattern_config)"></rect>
		...
		<div id="color_text_values">
			...
			<span class="h-hex-code">
				<svg id="color_picked_preview" width="20" height="20">
					<rect width="20" height="20" fill="#a7a7a7" rx="5" />
                </svg>
                <span data-hex-code>#1f1f1f</span>
			</span>
...
document.getElementById('color_picker_bg').addEventListener('click', function() {
    ...
    colorChange(`hsla(${colorPicker.hue}, ${colorPicker.saturation}%, ${colorPicker.lightness}%, ${colorPicker.alpha})`);
});
...
document.querySelectorAll('.hsla_input').forEach((element) => {
    element.addEventListener('change', function() {
        ...
        updateColorDisplays(`hsla(${hslaInput[0].value}, ${hslaInput[1].value}%, ${hslaInput[2].value}%, ${hslaInput[3].value})`);
    });
});
...
let colorSliderHandler = function(position) {
	...
	document.querySelector('span[data-hex-code]').innerHTML = HSLAToRGBA(colorPicker.hue, colorPicker.saturation, colorPicker.lightness, colorPicker.alpha, true);
    document.querySelector('.hype-cropper__body--image').style['background'] = HSLA;
...
let updateColorDisplays = function(color) {
	...
	document.querySelector('span[data-hex-code]').innerHTML = HSLAToRGBA(colorPicker.hue, colorPicker.saturation, colorPicker.lightness, colorPicker.alpha, true);
	document.querySelector('.hype-cropper__body--image').style['background'] = `hsla(${color.h}, ${color.s}%, ${color.l}%, ${color.a}`;
